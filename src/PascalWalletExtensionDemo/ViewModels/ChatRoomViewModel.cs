...
namespace PascalWalletExtensionDemo.ViewModels
{
    ...
    {
        private const uint ChatRoomType = 1001;
            ...
            set
            {
                ...
                switch (value.Method)
                {
                    ...
                    case PayloadMethod.Dest:
                        ...
                        {
                            _connectorHolder.Connector.GetAccountAsync(ReceiverAccount.AccountNumber).ContinueWith(task =>
                            {
        ...
        public List<Account> ChatRoomAccounts
        {
            ...
            set
            {
                if (_chatRoomAccounts != value)
                {
                    _chatRoomAccounts = value;
                    OnPropertyChanged(nameof(ChatRoomAccounts));
                }
            }
        }
        ...
        public Account ReceiverAccount
        {
            ...
            set
            {
                ...
                if(value != null)
                {
                    LoadMessages(value.AccountNumber).ContinueWith(task =>
                    {
                        if (task.Result != null)
                        {
                        ...
                        else
                        {
                            ...
                            InfoMessage = new InfoMessageViewModel("Failed to retrieve chatroom messages!", () => InfoMessage = null, true);
                        }
                    });

                    if (SelectedEncryptionMethod?.Method == PayloadMethod.Dest)
                    {
                        _connectorHolder.Connector.GetAccountAsync(value.AccountNumber).ContinueWith(task =>
                        {
        ...
        {
            return SenderAccount != null && ReceiverAccount != null && (!string.IsNullOrEmpty(Message) || Amount > 0);
        }
